...
class ItemsWithNotUpdatedStep extends \Hubleto\Erp\Controller
{
  ...
  public function prepareView(): void
  {
    ...
    try {
      ...
      $lastChangeInfoRaw = $mHistory->record
        ->where('model', $model)
        ->selectRaw('
          record_id as id,
          min(TIMESTAMPDIFF(DAY, datetime_change, now())) as last_change_before_days,
      ...
      $lastChangeInfo = [];
      foreach ($lastChangeInfoRaw as $value) {
        $lastChangeInfo[$value['id']] = $value['last_change_before_days'];
      }
      ...
      foreach ($records as $key => $record) {
        $records[$key]['last_change_before_days'] = $lastChangeInfo[$record['id']];
      }
