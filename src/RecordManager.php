...
// var_dump($main->getService(AuthProvider::class)->getUserType());
    ...
    $isTeamMember = false;
    // if ($hasIdTeam) {
    //   $isTeamMember = $main->getService(AuthProvider::class)->isUserMemberOfTeam($record['id_team']);
    // }
    ...
    if ($main->getService(AuthProvider::class)->getUserType() == User::TYPE_ADMINISTRATOR) {
      $canRead = true;
      $canModify = true;
    } if ($main->getService(AuthProvider::class)->getUserType() == User::TYPE_CHIEF_OFFICER) {
    ...
    } elseif ($main->getService(AuthProvider::class)->getUserType() == User::TYPE_MANAGER) {
      // Manager can:
    ...
    } elseif ($main->getService(AuthProvider::class)->getUserType() == User::TYPE_EMPLOYEE) {
      // Employee can:
    ...
    } elseif ($main->getService(AuthProvider::class)->getUserType() == User::TYPE_ASSISTANT) {
      // Assistant can:
      //   - read/modify only records where he/she is owner
    ...
    } elseif ($main->getService(AuthProvider::class)->getUserType() == User::TYPE_EXTERNAL) {
  ...
  public function prepareReadQuery(mixed $query = null, int $level = 0): mixed
  {
    ...
    $user = $main->getService(AuthProvider::class)->getUser();
    $userTeams = [];
    foreach ($user['TEAMS'] ?? [] as $team) {
      $userTeams[] = $team['id'] ?? 0;
    }

    if ($main->getService(AuthProvider::class)->getUserType() == User::TYPE_MANAGER) {
    ...
    } elseif ($main->getService(AuthProvider::class)->getUserType() == User::TYPE_EMPLOYEE && $hasIdOwner) {
      $query = $query->where($this->table . '.id_owner', $idUser);
    } elseif ($main->getService(AuthProvider::class)->getUserType() == User::TYPE_ASSISTANT && $hasIdOwner) {
      $query = $query->where($this->table . '.id_owner', $idUser);
    }
